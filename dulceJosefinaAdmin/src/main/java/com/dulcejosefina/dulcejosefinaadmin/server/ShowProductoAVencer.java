...
@WebServlet(name = "ShowProductoAVencer", urlPatterns = {"/ShowProductoAVencer"})
public class ShowProductoAVencer extends HttpServlet {

    @WebServiceRef(wsdlLocation = "WEB-INF/wsdl/localhost_8080/ServiceProducto/wsProducto.wsdl")
    private com.dulcejosefina.ejb.ServiceProducto service;
    ...
    try { // Call Web Service Operation
        ...
        xml = port.seleccionarProductosAVencer();
      ServletOutputStream servletOutputStream = response.getOutputStream();
            Document doc = reporte.obtenerDocumentoParseado(xml);
            byte[] bytes = reporte.obtenerReporteJasper(doc, "/Lista/item", reporte.obtenerReporteProductosAVencer());
            response.setContentType("application/pdf");
